...
public class PostData {
    ...
    public static String postDataWithImage(final Context context, String Url, HashMap<String, String> keyPaire, String[] path) {
        ...
        try {
            ...
            if (path.length == 1) {
                entitys.addPart("image", new FileBody(new File(path[0]), "image/jpg"));
            } else {
                for (int j = 0; j < path.length; j++) {
                    if (!path[j].equals("")) {
                        entitys.addPart("image_" + (j + 1), new FileBody(new File(path[j]), "image/jpg"));
                    }
                }
            }
    ...
    public static String getData(Context context, String url, HashMap<String, String> keyPaire) {
        if (checknetwork(context)) {
            ...
            Log.v("responce", " msg  " + urlResponce);
            System.out.print("-----generated url is----" + url);
            return urlResponce;
        } else {
            return context.getResources().getString(R.string.error_internet);
        }
